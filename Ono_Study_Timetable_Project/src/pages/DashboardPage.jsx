...
export default function DashboardPage() {
  ...
  const availableSemestersForSummary = useMemo(() => {
    if (!summaryFilters.yearCode) return [];
    const selectedYear = rawData.years.find(y => y.yearCode === summaryFilters.yearCode);
    return selectedYear?.semesters || [];
  }, [summaryFilters.yearCode, rawData.years]);
  ...
  const { upcomingEvents, upcomingTasks, pieChartData, summaryData, stats } = useMemo(() => {
    ...
    switch (upcomingTimeFilter) {
      ...
      case 'month': upEnd = endOfDay(addDays(now, 30)); break;
      case 'custom':
        upStart = upcomingCustomStart ? startOfDay(toValidDate(upcomingCustomStart)) : new Date(0);
        upEnd = upcomingCustomEnd ? endOfDay(toValidDate(upcomingCustomEnd)) : new Date(8640000000000000);
        break;
      default: upEnd = endOfDay(addDays(now, 7));
    }
    ...
    const upcomingEvents = (events || []).filter(ev => {
        ...
        return d && UPCOMING_EVENT_TYPES.has(type) && isWithinInterval(d, upInterval);
    ...
    const upcomingTasks = relevantTasks.filter(t => {
        ...
        return d && isWithinInterval(d, upInterval);
    ...
    if (yearCode) {
        ...
        if (year) {
            if (semesterCode) {
                definitionCount = courses.filter(c => c.semesterCode === semesterCode).length;
            } else {
                const yearSemesterCodes = new Set((year.semesters || []).map(s => s.semesterCode));
                definitionCount = courses.filter(c => yearSemesterCodes.has(c.semesterCode)).length;
            }
        }
    } else if (fStart || fEnd) {
        const s = fStart ? startOfDay(toValidDate(fStart)) : new Date(0);
        const e = fEnd ? endOfDay(toValidDate(fEnd)) : new Date(8640000000000000);
        const allSemesters = years.flatMap(y => y.semesters || []);
        const overlappingSemesters = allSemesters.filter(sem => {
            const semStart = toValidDate(sem.startDate);
            const semEnd = toValidDate(sem.endDate);
        ...
        definitionCount = courses.filter(c => overlappingSemesterCodes.has(c.semesterCode)).length;
    } else {
        definitionCount = courses.length;
    }
    ...
    if (yearCode) {
        ...
        if (year) {
            if (semesterCode) {
                const semester = (year.semesters || []).find(s => s.semesterCode === semesterCode);
                if (semester) {
                    interval = { start: toValidDate(semester.startDate), end: toValidDate(semester.endDate) };
                }
            } else {
                interval = { start: toValidDate(year.startDate), end: toValidDate(year.endDate) };
            }
        }
        
        if (interval && isValid(interval.start) && isValid(interval.end)) {
            timeFilteredEvents = events.filter(ev => {
                const d = toValidDate(ev.start);
                return d && isWithinInterval(d, interval);
            });
        } else {
            timeFilteredEvents = year ? [] : events; 
        }
    } else if (fStart || fEnd) {
        const s = fStart ? startOfDay(toValidDate(fStart)) : new Date(0);
        const e = fEnd ? endOfDay(toValidDate(fEnd)) : new Date(8640000000000000);
    ...
    const summaryEventCounts = timeFilteredEvents.reduce((acc, ev) => {
        const type = ev.extendedProps?.type || 'default';
        if(type !== 'courseDefinition') { acc[type] = (acc[type] || 0) + 1; }
        return acc;
    }, {});
  ...
  // ✨ FIX 1: The correct, simplified logic for the filter handler.
  const handleSummaryFilterChange = (e) => {
    ...
    setSummaryFilters(prev => {
      ...
      if (name === 'yearCode') {
        newState.semesterCode = ''; // Reset semester when year changes
      }
      return newState;
    });
  };
  ...
  const handleUpcomingFilterChange = (e) => {
    setUpcomingTimeFilter(e.target.value);
    if (e.target.value !== 'custom') {
      setUpcomingCustomStart('');
      setUpcomingCustomEnd('');
    }
  };
  ...
  // ✨ FIX 2: Sort years chronologically ascending.
