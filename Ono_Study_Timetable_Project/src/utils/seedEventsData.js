...
const generateCourseMeetings = (courses, semesters, holidays, vacations) => {
  ...
  courses.forEach((course, courseIndex) => {
    ...
    if (!semester) { console.warn(`[Generate Meetings] Course ${course.courseCode}: Semester ${semester.semesterCode} not found.`); return; }
    if (!semester.startDate || !semester.endDate) { console.warn(`[Generate Meetings] Course ${course.courseCode}: Semester ${semester.semesterCode} has invalid dates.`); return; }
    if (!Array.isArray(course.hours) || course.hours.length === 0) { console.warn(`[Generate Meetings] Course ${course.courseCode}: Missing or empty 'hours' array.`); return; }
    try {
      const start = new Date(semester.startDate + 'T00:00:00Z');
      const end = new Date(semester.endDate + 'T00:00:00Z');
      if (isNaN(start.getTime()) || isNaN(end.getTime())) { console.warn(`[Generate Meetings] Course ${course.courseCode}: Could not parse semester dates.`); return; }
...
export const seedEventsData = async (force = false) => {
  ...
  try {
    ...
    if (force || !localStorage.getItem("coursesMeetings")) {
      ...
      saveRecords("coursesMeetings", generatedMeetings);
